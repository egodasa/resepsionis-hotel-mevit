...
class jenisUjian extends Controller
{
    /**
     ...
     */
    public $tablePk = 'id_jujian';
    public $table = "tbjenis_ujian";
    ...
    public function store(Request $req, $id = null) //insert  dan update disatu method
    {
        $data = new \stdclass;
		$insert = [
			"nm_jujian"		=> $req->input('nm_jujian')
		];
		$validate = [
			"nm_jujian"		=> "bail|required|max:20",
	    ];
		$validator = \Validator::make($insert, $validate);
		if($validator->fails()){
			$errors = $validator->errors();
			$data->error = [
				"nm_jujian"	=> $errors->first('nm_jujian')
			];
			$data->status_code = "422";
		}else{
